...
You can look up the details of these options in the [MSDN Reference](http://msdn.microsoft.com/en-us/library/microsoft.xna.framework.graphics.spritesortmode.aspx). SpriteSortMode.Immediate is required to apply the effect. BlendState.AlphaBlend is the default. After your Begin call we're going to add the following line which will Apply the pixel shader to the sprite.
